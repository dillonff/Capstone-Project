...
const Chat = () => {
  ...
  useEffect(
    (_) => {
      if (currentWorkspace.id === -1) return;
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center' }}>
          ...
          <input
            ...
            onClick={(_) => {
              ...
              if (name) {
                ...
                req = JSON.stringify(req);
                callApi('/workspaces', 'POST', auth.current, req)
                  .then((res) => {
                    if (!res.ok) {
                      console.error(res);
      ...
      <div style={{ width: '400px', margin: 'auto', padding: '5px' }}>
        ...
        <div>
          Channel ({currentChannel.id}): {currentChannel.name}
        </div>
        ...
        >
          <input
            type="text"
            value={msgToSend}
            onInput={(e) => setMsgToSend(e.target.value)}
          ></input>
          <input
            type="button"
            value="Send"
            onClick={() => {
              ...
              callApi('/messages/send', 'POST', auth.current, body).then(
                (res) => {
                  if (res.ok) {
                    setMsgToSend('');
                  } else {
